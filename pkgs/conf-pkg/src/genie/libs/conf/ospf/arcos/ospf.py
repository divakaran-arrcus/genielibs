#!/usr/bin/env python3
"""
Native ArcOS OSPFv2 configuration plugin for Genie.

This implementation provides full ArcOS OSPFv2 CLI generation under the
standard Genie conf OSPF abstraction. It follows the ISIS conf object as
the gold-standard pattern, adapted for the OSPF area-based hierarchy:
...
All three phases of attributes are included:
  ...
  Phase 2 — Hardening (auth, BFD, priority, route-preference, summary-aggregate)
  Phase 3 — Advanced (SPF throttle, LSA timers, max-LSA, maintenance mode,
             redistribute-aggregate, SNMP traps)
"""
...
class Ospf(ABC):
    """ArcOS-specific OSPFv2 implementation for Genie (native plugin)."""

    class DeviceAttributes(ABC):
        """Device-level OSPF attributes for ArcOS."""
        ...
        # ============================================================
        # Phase 3 — Redistribute aggregates (global level)
        # ============================================================
        ...
        redistribute_aggregates = managedattribute(
            ...
            doc='Dict of prefix -> {advertise: str, import_policy: str}')

        # ============================================================
        # Phase 3 — SNMP traps (optional)
        # ============================================================

        snmp_traps = managedattribute(
            name='snmp_traps',
            default=None,
            type=(None, managedattribute.test_istype(dict)),
            doc='Dict of trap-name -> bool, e.g. {"if-auth-failure": true}')
        ...
        def build_config(self, apply=True, attributes=None, unconfig=False, **kwargs):
            """Build OSPFv2 configuration for an ArcOS device.
            ...
            if unconfig:
                configurations.append_line(
                    f'network-instance {instance_name} protocol OSPF {pid}'
                )
            else:
                with configurations.submode_context(
                    ...
                    with configurations.submode_context(
                        ...
                        # ========================================
                        # PHASE 3 — Redistribute aggregates
                        # ========================================
                        ...
                        snmp = attributes.value('snmp_traps')
                        if snmp and isinstance(snmp, dict):
                            for trap_name in sorted(snmp.keys()):
                                trap_val = snmp[trap_name]
                                trap_str = 'true' if trap_val else 'false'
                                configurations.append_line(
                                    f'global snmp send-trap {trap_name} '
                                    f'{trap_str}'
                                )
        ...
        class AreaAttributes(ABC):
            """Area-level OSPF attributes for ArcOS."""
            ...
            # Phase 2 — Summary aggregates (area level)
            ...
            def build_config(
                ...
                **kwargs,
                ...
                """Build area-level OSPF configuration."""
                assert not kwargs, "Unexpected kwargs: {}".format(kwargs)
                attributes = AttributesHelper(self, attributes)
                configurations = CliConfigBuilder(unconfig=unconfig)
            ...
            class InterfaceAttributes(ABC):
                """Interface-specific OSPF attributes for ArcOS."""
                ...
                auth_type = managedattribute(
                    ...
                    default=None,
                    type=(None, managedattribute.test_istype(str)),
                    doc='Auth type: OSPF_AUTH_NULL or OSPF_AUTH_CRYPTO_KEY')

                auth_crypto_key_id = managedattribute(
                    name='auth_crypto_key_id',
                    default=None,
                    type=(None, managedattribute.test_istype(int)),
                    doc='Crypto key ID (0-255, default 0)')

                auth_crypto_algorithm = managedattribute(
                    name='auth_crypto_algorithm',
                    default=None,
                    type=(None, managedattribute.test_istype(str)),
                    doc='Crypto algorithm: OSPF_CRYPTO_ALGO_MD5')

                auth_crypto_key_string = managedattribute(
                    name='auth_crypto_key_string',
                    default=None,
                    type=(None, managedattribute.test_istype(str)),
                    doc='Key string for crypto digest (max 16 for MD5)')
                ...
                def build_config(
                    ...
                    **kwargs,
                    ...
                    """Build interface-specific OSPF configuration."""
                    assert not kwargs, "Unexpected kwargs: {}".format(kwargs)
                    attributes = AttributesHelper(self, attributes)
                    configurations = CliConfigBuilder(unconfig=unconfig)

                    intf_name = (
                        attributes.value('interface_id')
                    ...
                    with configurations.submode_context(
                        ...
                        a_type = attributes.value('auth_type')
                        if a_type:
                            configurations.append_line(
                                f'authentication auth-type {a_type}'
                            )

                        key_id = attributes.value('auth_crypto_key_id')
                        if key_id is not None:
                            configurations.append_line(
                                f'authentication crypto-key key-id {key_id}'
                            )

                        algo = attributes.value('auth_crypto_algorithm')
                        if algo:
                            configurations.append_line(
                                f'authentication crypto-key algorithm {algo}'
                            )

                        key_str = attributes.value('auth_crypto_key_string')
                        if key_str:
                            configurations.append_line(
                                'authentication crypto-key key-string '
                                f'{key_str}'
                            )
                ...
                def build_unconfig(
                    ...
                    """Build interface-level OSPF unconfiguration."""
                    return self.build_config(
                        apply=apply,
                        attributes=attributes,
                        unconfig=True,
